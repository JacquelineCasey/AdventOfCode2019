...
pub fn main() !void {
    ...
    var screen = std.AutoHashMap(struct {i64, i64}, Tile).init(alloc);
    defer screen.deinit();

    while (computer.pop_output()) |x| {
        const y = computer.pop_output().?;
        const tile: Tile = @enumFromInt(computer.pop_output().?);
        
        try screen.put(.{x, y}, tile);
    }

    var count: u32 = 0;
    var iter = screen.valueIterator();
    while (iter.next()) |val| {
        if (val.* == .Block) {
            count += 1;
        }
