...
pub fn main() !void {
    ...
    var scaffold_locations = std.AutoHashMap(Pair, void).init(alloc);
    defer scaffold_locations.deinit();

    var x: i32 = 0;
    var y: i32 = 0;
    while (computer.pop_output()) |ch| {
        if (ch == '#') {
            try scaffold_locations.put(.{x, y}, {});
        }
    ...
    var sum: i32 = 0;
    var iter = scaffold_locations.keyIterator();

    while (iter.next()) |pair| {
        x = pair[0];
        y = pair[1];

        if (scaffold_locations.contains(.{x+1, y}) and scaffold_locations.contains(.{x-1, y}) 
                and scaffold_locations.contains(.{x, y+1}) and scaffold_locations.contains(.{x, y+1})) {
