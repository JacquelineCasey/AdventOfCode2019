...
// Had to change to i32... probably should have done that in the first place...
...
fn angle(diff_pair: Pair) f64 {
    // We treat x as y and vice versa in order to get our clockwise behavior.
    return @mod(
        std.math.atan2(
            f64, 
            @as(f64, @floatFromInt(diff_pair.x)), 
            @as(f64, @floatFromInt(-diff_pair.y)) // Remember, up is negative
        ), 
        (std.math.pi * 2)
    );
}

fn angle_cmp(_: void, pair_1: Pair, pair_2: Pair) bool {
    return angle(pair_1) < angle(pair_2);
}
...
pub fn main() !void {
    ...
    var max_val: u32 = 0;
    var maybe_laser_pair: ?Pair = null;
    var entry_iter = pairs.iterator();
    while (entry_iter.next()) |entry| {
        if (entry.value_ptr.* > max_val) {
            max_val = entry.value_ptr.*;
            maybe_laser_pair = entry.key_ptr.*;
        }
    }

    const laser_pair = maybe_laser_pair.?;

    if (max_val < 200) return error.MultipleCyclesNeeded;

    // Filter pairs that are not visible. We will store (possibly not reduced) slope pairs from now on.
    var visible_pairs = std.ArrayList(Pair).init(alloc);
    defer visible_pairs.deinit();

    iter_1 = pairs.keyIterator();
    next_pair: while (iter_1.next()) |pair| {
        const x_1 = laser_pair.x;
        const y_1 = laser_pair.y;
        ...
        // pair is visible from laser_pair
        try visible_pairs.append(.{.x = x_2 - x_1, .y = y_2 - y_1});
    } 

    std.sort.heap(Pair, visible_pairs.items, {}, angle_cmp);
    
    const bet_pair = visible_pairs.items[200 - 1];

    try std_out.print("{d}\n", .{(bet_pair.x + laser_pair.x) * 100 + (bet_pair.y + laser_pair.y)});
}
