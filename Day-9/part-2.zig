...
// We swap out the array memory with a hashtable
// We also start using i64
...
pub fn main() !void {
    ...
    try computer.push_input(2);

    try computer.run();

    while (computer.pop_output()) |item| {
        try std_out.print("{d}\n", .{item});
    }
}

// Changing to hashtable memory was a little annoying, since it meant propagating
// fallability everywhere (added like 20 trys).

// I believe part 2 is about optimization. It seems to hint that things could get
// slow. I bet people who allocated tons of empty memory got punished here, but 
// my hashtable approach avoided that issue altogether.

// Overall, day 9 was pretty easy.
