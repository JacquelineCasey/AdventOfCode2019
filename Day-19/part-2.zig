...
fn test_location(x: u32, y: u32, base_memory: std.AutoHashMap(usize, i64), alloc: std.mem.Allocator) !bool {
    var computer = IntCodeComputer.init(try base_memory.clone(), alloc);
    defer computer.deinit();

    try computer.push_input(@intCast(x));
    try computer.push_input(@intCast(y));
...
pub fn main() !void {
    ...
    // x represents leftmost location on row y.
    var x: u32 = 0;
    var y: u32 = 99;
    while (true) : (y += 1) {
        // Shift x
        while (!try test_location(x, y, base_memory, alloc)) : (x += 1) {}

        if (try test_location(x + 99, y - 99, base_memory, alloc))
            break;
    }
...
// Pretty easy... The main futzing I did was to try to figure out if I could avoid
// an allocation each time I run the computer. And I could, but not really in a
// simple way. What I have now is fast enough regardless.

// Kinda a cool question, but I thought I would have to do binary search or get
// the boundries for each row and do something cool with them. Instead, it is
// sufficient to simply trace along the left boundary, and test the square defined
