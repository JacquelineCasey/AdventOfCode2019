...
pub fn main() !void {
    ...
    var last_pushed_Y: ?i64 = null;

    var idle_counts: [50] u32 = .{0} ** 50;
    while (true) {
        for (0..50) |i| {
        ...
        if (num_idle == 50) {
            ...
            try computers[0].push_input(X);
            try computers[0].push_input(Y);

            if (last_pushed_Y) |val| {
                if (val == Y) {
                    try std_out.print("{d}\n", .{Y});
                    return;
                }
            ...
            computers[0].state = .Runnable;
        }
    }
}

// Weird. If the NAT waits 200 rounds before sending out a packet, things work.
// Otherwise, it ends up being too agressive, and I don't really know why.

// I had an awful bug where I grabbed a computer out of the array, assuming that
// I was doing reference semantics. Ultimately, not all of the changes I made to
// that computer propagated to the one in the array, but I think some of them did?
// Kinda a yikes there...

// Probably I could have made the computers run faster, but the problem was really
// vague about if they should run in sync or not... Kinda not a fan of this problem
// as a result, it felt kinda wishy washy.

// I think if you had gotten this far without fully wrapping your computers with
// input queues and states and stuff, this could be a lot harder and therefore more
// interesting. I did change my computer a little, giving the queues a len opeartion
// and allowing the computers to run instructions one at a time, but my design made
// these things fairly trivial.